...
def train_detector(args,
    ...
    for i, data in enumerate(train_loader):
        ...
        length = length + torch.tensor((torch.rand(length.size(0))-0.5) * args.random_length, dtype=torch.int32)
        length = torch.clamp(length,min=0,max=max_length)
        ...
        output = predictor(masked_frames,length)
        ...
        output_r = predictor(reverse_frames,length)


        loss_classify_r = args.reverse_weight * F.mse_loss(F.softmax(output_r,dim=1),F.softmax(torch.ones_like(output_r)))
        ...
        output = predictor(frames,length)
...
def test_detector(args,test_loader,detector,predictor):
    detector.eval()
    predictor.eval()
    acc_mask_avg = 0
    acc_r_avg = 0
